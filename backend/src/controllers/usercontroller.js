...
// List users (admin, csr, or technician can list customers)
...
exports.createUser = async (req, res, next) => {
  try {
    ...
    if (!['csr', 'technician', 'admin', 'contractor', 'superadmin', 'hr', 'customer', 'procurement', 'finance'].includes(role)) return res.status(400).json({ message: 'Invalid role' });
    ...
    if (ipAddress) userData.ipAddress = ipAddress;
