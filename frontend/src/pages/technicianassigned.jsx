// src/pages/TechAssigned.js
import React, { useEffect, useState } from 'react';
import API from '../utils/api';
import { useNavigate } from 'react-router-dom';

export default function TechAssigned() {
  ...
  useEffect(() => {
    const fetchTickets = () => {
      API.get('/tickets?assignedTo=' + (JSON.parse(localStorage.getItem('user') || '{}')._id || '')).then(res => {
        let payload = [];
  ...
  return (
    <>
      <div className="technician-dashboard-container" style={{
        padding: 32,
        marginTop: 56,
        minHeight: '100vh',
        ...
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <h1 style={{ color: '#2d7a3e', margin: 0, fontSize: 32, fontWeight: 800, textShadow: '0 2px 8px rgba(0,0,0,0.1)', letterSpacing: 1 }}>Technician Dashboard</h1>
          <p style={{ color: '#4a4a4a', fontSize: 16, marginTop: 8, opacity: 0.8 }}>Manage your assigned customer tickets</p>
        </div>

        {/* Search bar for technician */}
        ...
        <div style={{ marginTop: 32 }}>
          <h2 style={{ color: '#2d7a3e', marginTop: 0, marginBottom: 20, textAlign: 'center', fontSize: 28, fontWeight: 700 }}>My Assigned Tickets</h2>
          <div className="technician-tickets-container" style={{ display: 'grid', gap: 40 }}>
            {tickets.length === 0 ? (
              <div style={{ textAlign: 'center', padding: 60, background: 'linear-gradient(135deg, #f7fff7 0%, #e8f5e9 100%)', borderRadius: 16, border: '2px dashed #43e97b' }}>
                ...
                <div style={{ fontSize: 20, color: '#2d7a3e', fontWeight: 700, marginBottom: 8 }}>No assigned tickets</div>
                <div style={{ fontSize: 16, color: '#4a4a4a', opacity: 0.8 }}>Your assigned tickets will appear here</div>
              </div>
            ) : tickets
              ...
              .map(t => (
                <div
                  ...
                  style={{
                    ...
                    boxShadow: '0 8px 32px rgba(255, 152, 0, 0.12), 0 4px 12px rgba(44,62,80,0.08)',
                    display: 'flex',
                    alignItems: 'center',
